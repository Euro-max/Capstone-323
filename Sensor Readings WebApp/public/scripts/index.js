...
function epochToJsDate(epochTime){
  ...
  const moiElement = document.getElementById("moi");
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      // Get number of readings to plot saved on database (runs when the page first loads and whenever there's a change in the database)
        ...
        dbRef.orderByKey().limitToLast(60).on('child_added', snapshot =>{
          var jsonData = snapshot.toJSON(); // example: {temperature: 25.02, humidity: 50.20, pressure: 1008.48, timestamp:1641317355}
          // Save values on variables
          var AirTemperature = jsonData.AirTemperature;
          var SoilTemperature = jsonData.SoilTemperature;
          var Moisture = jsonData.Moisture;
          var Timestamp = jsonData.Timestamp;
          // Plot the values on the charts
          plotValues(chartAT, Timestamp, AirTemperature);
          plotValues(chartST, Timestamp, SoilTemperature);
          plotValues(chartM, Timestamp, Moisture);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        var Timestamp = jsonData.Timestamp;
        // Update DOM elements
        tempElement.innerHTML = AirTemperature;
        stElement.innerHTML = SoilTemperature;
        moiElement.innerHTML = Moisture;
      ...
      function createTable(){
        ...
        dbRef.orderByKey().limitToLast(100).on('child_added', function(snapshot) {
          if (snapshot.exists()) {
            ...
            content += '<td>' + AirTemperature + '</td>';
            content += '<td>' + SoilTemperature + '</td>';
            content += '<td>' + Moisture + '</td>';
            content += '</tr>';
          ...
          if (snapshot.exists()) {
            ...
            reversedList.forEach(element =>{
              if (firstTime){ // ignore first reading (it's already on the table from the previous query)
                ...
                var AirTemperature = element.AirTemperature;
                var SoilTemperature = element.SoilTemperature;
                var Moisture = element.Moisture;
